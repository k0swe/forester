...
describe('GeocodeService', () => {
  ...
  let geocoderSpy: { geocode: Mock };

  beforeEach(() => {
    geocoderSpy = { geocode: vi.fn() };
  ...
  describe('geocode()', () => {
    it('should geocode by lat/lon when coordinates are present', async () => {
      geocoderSpy.geocode.mockReturnValue(
        of(makeGeocoderResponse(usComponents)),
      );
    ...
    it('should use long state name for a non-US address', async () => {
      geocoderSpy.geocode.mockReturnValue(
        of(makeGeocoderResponse(caComponents)),
      );
      const result = await service.geocode({
        latitude: 43.4643,
        longitude: -80.5204,
