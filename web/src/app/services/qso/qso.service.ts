...
export class QsoService implements OnDestroy {
  private readonly firestore: Firestore = inject(Firestore);
  private readonly auth: Auth = inject(Auth);

  private currentBook = '';
  private readonly _qsos$ = new BehaviorSubject<FirebaseQso[]>([]);
  private readonly _filterCriteria$ = new BehaviorSubject<FilterCriteria>({});
  private unsubscribe: (() => void) | undefined;

  /**
   * Observable stream of all QSOs in the current logbook.
   */
  public readonly qsos$: Observable<FirebaseQso[]> = this._qsos$.asObservable();

  /**
   * Converts string/number dates to Date objects for a QSO.
   */
  private static unmarshalDates(qso: Qso): void {
    if (qso.timeOn != null) {
      qso.timeOn = new Date(qso.timeOn as string | number | Date);
    }
    if (qso.timeOff != null) {
      qso.timeOff = new Date(qso.timeOff as string | number | Date);
    }
  }

  /**
   * Converts Date objects to ISO strings for a QSO.
   */
  private static marshalDates(qso: Qso): void {
    if (qso.timeOn != null && qso.timeOn instanceof Date) {
      // @ts-ignore
      qso.timeOn = qso.timeOn.toISOString();
    }
    if (qso.timeOff != null && qso.timeOff instanceof Date) {
      // @ts-ignore
  ...
  public init(bookCall: string): void {
    ...
    user(this.auth).subscribe((_) => {
      ...
      this.unsubscribe = onSnapshot(contactsCollection, (qsosSnapshot) => {
        this._qsos$.next(this.unpackDocs(qsosSnapshot));
      });
    });
  }
  ...
  getFilteredQsos(): Observable<FirebaseQso[]> {
    return combineLatest([this._qsos$, this._filterCriteria$]).pipe(
  ...
  findWASQso(criteria: WASQsoCriteria): Observable<FirebaseQso | undefined> {
    return this._qsos$.pipe(
      map((qsos) => {
        const matches = Array.from(qsos.values())
          .sort((a, b) => {
            const aTime = a.qso.timeOn?.getTime() ?? Number.POSITIVE_INFINITY;
            const bTime = b.qso.timeOn?.getTime() ?? Number.POSITIVE_INFINITY;
            return aTime - bTime;
          })
   ...
   */
  public addOrUpdate(fbq: FirebaseQso): Observable<void | { id: string }> {
    ...
    return user(this.auth).pipe(
      mergeMap((u) => {
        ...
        if (fbq.id == null) {
          ...
          return from(addDoc(contactsCollection, fbq.qso!)).pipe(
    ...
    const match = this._qsos$
      ...
      .find(
        (fbq) =>
          ...
          fbq.qso.loggingStation?.stationCall ===
            qso.loggingStation?.stationCall,
      );
    return match !== undefined;
  }

  delete(firebaseId: string): Observable<void> {
    return user(this.auth).pipe(
      mergeMap((u) => {
        if (u == null) {
          return of();
